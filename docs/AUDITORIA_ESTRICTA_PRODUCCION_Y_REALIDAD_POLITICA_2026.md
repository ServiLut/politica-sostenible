...
> Corte de evidencia: 7 de septiembre de 2026.
> Alcance: aplicación web, API, operación política colombiana y riesgos de salida a producción.
> Veredicto: **el despliegue observado no está listo para una operación política crítica**. El candidato local incorpora los cierres descritos y se congela en una rama local identificable, pero aún no tiene CI remoto verde ni promoción aprobada. Exige además inventario de la base real, restauración ensayada, staging equivalente y pruebas con un tenant desechable antes de reemplazar producción.
...
| **P0**    | **Cadena de suministro sin congelar.** No se puede atribuir ni reproducir un binario político crítico desde un árbol mutable.                     | El remoto visible en [GitHub](https://github.com/ServiLut/politica-sostenible) es público, `main` no está protegido, las cinco ejecuciones CI más recientes fallaron y producción no expone commit/digest verificable. | No se encontró un `.env` ni secreto de servidor vivo en el historial revisado; sí existió una clave pública Supabase `anon` en un Dockerfile antiguo. El árbol local se aisló y congeló en una rama de release; todavía requiere remoto privado y CI verde. | Volver privado el remoto, verificar políticas del proyecto Supabase histórico, rotar preventivamente credenciales, proteger `main`, conservar commit/digest y registrar exactamente qué artefacto se despliega. |
...
Estas correcciones están en el repositorio local y superaron la validación técnica local descrita abajo. Eso no significa que estén desplegadas ni que se hayan probado con credenciales, proveedores y datos reales:
...
Estas cifras corresponden al árbol local auditado, no a la versión vieja actualmente desplegada. Deben repetirse en CI sobre un commit congelado y en staging antes del corte.
...
2. **Gates verdes:** pruebas unitarias, integración PostgreSQL, HTTP e2e, Playwright desktop/móvil, builds y escaneo de dependencias repetidos sobre el commit final.
...
La decisión responsable es **no desplegar directamente sobre producción ni declarar terminado el producto**. Debe mantenerse inmutable el commit candidato, repetir todos los gates en CI, inventariar la base real, restaurar y ensayar su backup, desplegar el mismo digest en un ambiente equivalente con tenant desechable y completar los escenarios críticos. El corte posterior debe hacerse en mantenimiento, drenando la versión anterior e iniciando únicamente el mismo commit que migró la base; no mediante rollout gradual de binarios incompatibles.
