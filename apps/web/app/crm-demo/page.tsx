...
const AppContext = createContext<{
  ...
  addReport: (
    report: Omit<E14Report, "id" | "timestamp" | "testigoId">,
  ) => void;
  massImport: () => void;
  updateMissionStatus: (
    id: number,
...
const generateInitialData = (): AppState => {
    ...
    role:
      i === 0
        ...
        : i === 1
          ...
          : i === 49
            ? "Testigo"
            : (["Gerente", "LiderZonal"][i % 2] as Role),
    zona: BARRIOS[i % 10],
  }));
  ...
  const voters: Voter[] = Array.from({ length: 200 }).map((_, i) => ({
    ...
    liderId:
      users.find((u) => u.role === "LiderZonal" && u.zona === BARRIOS[i % 10])
        ?.id || 1,
  }));
  ...
  const alerts: Alert[] = [
    {
      id: 1,
      msg: "Anomalía reportada en Mesa 5: 0 votos registrados",
      time: "16:10",
      type: "fraud",
      severity: "high",
    },
    {
      id: 2,
      msg: "Testigo Mesa 12 no ha reportado asistencia",
      time: "08:00",
      type: "operational",
      severity: "medium",
    },
  ...
  return {
    ...
    logs: [
      {
        id: 1,
        msg: "Sistema de War Room activado",
        time: "1 min",
        type: "info",
      },
    ],
...
const DashboardView = () => {
  ...
  const filteredVoters = useMemo(() => {
    if (currentUser.role === "SuperAdmin" || currentUser.role === "Gerente")
      return voters;
    if (currentUser.role === "LiderZonal")
      return voters.filter((v) => v.barrio === currentUser.zona);
    return [];
  }, [currentUser, voters]);

  const totalVotosSeguros = filteredVoters.filter(
    (v) => v.intencion >= 4,
  ).length;
  const pendingMissions = state.missions.filter(
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          {
            label: "Votos Fidelizados",
            val: totalVotosSeguros,
            icon: UserCheck,
            color: "text-blue-600",
            bg: "bg-blue-50",
          },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
          <h3 className="font-bold text-lg mb-6">
            Crecimiento de Votos vs. Tiempo
          </h3>
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
          ...
          <div className="space-y-4">
            {logs.map((log) => (
              <div
                key={log.id}
                className="flex gap-3 pb-4 border-b border-slate-50 dark:border-slate-700 last:border-0"
              >
                <div
...
const MissionsView = () => {
  ...
  const leaderPerformance = useMemo(() => {
    return users
      .filter((u) => u.role === "LiderZonal")
      .map((u) => {
        const leaderMissions = missions.filter((m) => m.assignedTo === u.id);
        const completed = leaderMissions.filter(
          (m) => m.status === "Completada",
        ).length;
        return {
          name: u.name,
          ...
          rate:
            leaderMissions.length > 0
              ? (completed / leaderMissions.length) * 100
              : 0,
        };
      })
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500">
            Automatización de tareas por reglas de fidelización
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-2xl border">
          <h3 className="font-bold mb-4 flex items-center gap-2">
            <TrendingUp className="h-5 w-5 text-blue-600" /> Ranking de
            Cumplimiento
          </h3>
          <div className="space-y-4">
        ...
        <div className="bg-blue-600 p-6 rounded-2xl text-white">
          ...
          <div className="mt-4 flex justify-between text-xs opacity-80 uppercase font-bold tracking-widest">
            <span>
              Pendientes:{" "}
              {missions.filter((m) => m.status === "Pendiente").length}
            </span>
            <span>
              Completas:{" "}
              {missions.filter((m) => m.status === "Completada").length}
            </span>
          </div>
        </div>
      </div>
            ...
            {missions.slice(0, 50).map((m) => {
              ...
              return (
                <tr key={m.id} className="hover:bg-slate-50">
                  ...
                  <td className="p-4">
                    <span
                      className={`px-2 py-1 rounded-full text-[10px] font-bold ${getStatusColor(m.status)}`}
                    >
                      {m.status}
                    </span>
                  </td>
                  <td className="p-4">
                    <div className="flex gap-2">
                      {m.status === "Pendiente" && (
                        <button
                          onClick={() =>
                            updateMissionStatus(m.id, "En Proceso")
                          }
                      ...
                      {m.status === "Validación" && (
                        <button
                          onClick={() =>
                            updateMissionStatus(m.id, "Completada")
                          }
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl p-8 shadow-2xl">
            ...
            <p className="text-slate-500 text-sm mb-6">
              Obligatorio: Describe qué sucedió durante la misión para poder
              pasar a validación.
            </p>
            <textarea
...
const MessagingView = () => {
  ...
  const audience = useMemo(() => {
    return voters.filter((v) => {
      const barrioMatch =
        selectedBarrios.length === 0 || selectedBarrios.includes(v.barrio);
      const intencionMatch =
        selectedIntenciones.length === 0 ||
        selectedIntenciones.includes(v.intencion);
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500">
            Comunicaciones masivas personalizadas por zona e intención
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border space-y-4">
            <h4 className="font-black text-xs uppercase text-slate-400 tracking-widest">
              ...
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((i) => (
                  <button
                    key={i}
                    onClick={() => {
                      if (selectedIntenciones.includes(i))
                        setSelectedIntenciones(
                          selectedIntenciones.filter((x) => x !== i),
                        );
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center bg-slate-50 p-4 rounded-2xl">
                <div className="flex gap-4">
                  <button className="text-xs font-bold flex items-center gap-1 text-slate-500">
                ...
                <button
                  disabled={
                    !campaignName || !messageText || audience.length === 0
                  }
                  onClick={() => {
                    createCampaign({
                      name: campaignName,
                      ...
                      segment: {
                        ...
                        intenciones: selectedIntenciones,
                      },
                    });
                    setCampaignName("");
                    setMessageText("");
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border">
            <h4 className="font-black text-xs uppercase text-slate-400 tracking-widest mb-4">
              Historial de Envíos
            </h4>
            <div className="space-y-3">
              {messages.map((m) => (
                <div
                  key={m.id}
                  className="flex justify-between items-center p-3 hover:bg-slate-50 rounded-xl border border-transparent hover:border-slate-100 transition-all"
                >
                  <div>
...
const VoterDirectory = () => {
  ...
  if (["Contador", "Testigo"].includes(currentUser.role))
    return (
      <div className="h-full flex flex-col items-center justify-center space-y-4">
        <ShieldAlert className="h-16 w-16 text-red-500" />
        <h2 className="text-2xl font-bold">Acceso Denegado</h2>
        <p className="text-slate-500">
          Su rol no tiene permisos para ver el censo electoral.
        </p>
      </div>
    );
  ...
  const filteredVoters = useMemo(() => {
    let result = voters;
    if (currentUser.role === "LiderZonal")
      result = result.filter((v) => v.barrio === currentUser.zona);
    if (searchTerm) {
    ...
    if (filters.barrio.length > 0)
      result = result.filter((v) => filters.barrio.includes(v.barrio));
    if (filters.intencion.length > 0)
      result = result.filter((v) => filters.intencion.includes(v.intencion));
    if (filters.gender.length > 0)
      result = result.filter((v) => filters.gender.includes(v.gender));
    return result;
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Mostrando {filteredVoters.length.toLocaleString()} registros totales
          </p>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              {[5, 4, 3, 2, 1].map((lvl) => (
                <label
                  key={lvl}
                  className="flex items-center gap-2 text-sm cursor-pointer hover:text-blue-600 group"
                >
                  <input
                    type="checkbox"
                    className="rounded text-blue-600"
                    onChange={(e) => {
                      const newInt = e.target.checked
                        ? [...filters.intencion, lvl]
                        : filters.intencion.filter((x) => x !== lvl);
                      setFilters({ ...filters, intencion: newInt });
                      setCurrentPage(1);
                    }}
                  />
                  <span
                    className={`h-2 w-2 rounded-full ${lvl >= 4 ? "bg-green-500" : lvl === 3 ? "bg-yellow-500" : "bg-red-500"}`}
                  />
                  {lvl === 5
                    ? "Voto Duro (A+)"
                    : lvl === 4
                ...
                {BARRIOS.map((b) => (
                  ...
                  >
                    <input
                      ...
                      className="rounded text-blue-600"
                      onChange={(e) => {
                        const newBarrio = e.target.checked
                          ? [...filters.barrio, b]
                          : filters.barrio.filter((x) => x !== b);
                  ...
                  >
                    <td className="p-4">
                      <select
                        ...
                        value={v.intencion}
                        onChange={(e) =>
                          handleIntentionChange(v.id, Number(e.target.value))
                        }
                      >
...
const FinanceModule = () => {
  ...
  if (["LiderZonal", "Testigo"].includes(currentUser.role))
    return (
      <div className="h-full flex flex-col items-center justify-center space-y-4">
        <ShieldAlert className="h-16 w-16 text-red-500" />
        <h2 className="text-2xl font-bold">Módulo Restringido</h2>
        <p className="text-slate-500">
          Solo personal administrativo puede ver las finanzas.
        </p>
      </div>
    );
...
const TerritoryModule = () => {
  ...
  const barrioStats = useMemo(() => {
    return BARRIOS.map((barrio) => {
      ...
      const percentage =
        votersInBarrio.length > 0
          ? (duroCount / votersInBarrio.length) * 100
          : 0;
      let color =
        percentage >= 70
          ? "bg-emerald-500"
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {barrioStats.map((b, i) => (
          <div
            key={i}
            className={`${b.color} p-6 rounded-2xl text-white shadow-lg flex flex-col items-center justify-center text-center`}
          >
...
const ElectionDayModule = () => {
  ...
  const totalVotes = useMemo(
    () => reports.reduce((acc, curr) => acc + curr.votosCandidato, 0),
    [reports],
  );
  ...
  if (currentUser.role === "Contador")
    return (
      <div className="p-10 text-center">Sin acceso al War Room electoral.</div>
    );
    ...
    <div className="min-h-screen bg-slate-950 text-white p-8 space-y-8 font-mono border-4 border-red-900/20 rounded-3xl animate-in zoom-in-95 duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 border-b border-red-900/30 pb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-4xl font-black tracking-tighter text-red-500 uppercase">
              War Room E-14
            </h2>
            <p className="text-xs text-red-900 font-bold uppercase tracking-[0.2em]">
              Live Operation: Colombia 2026
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {isWitness || showForm ? (
            <div className="bg-slate-900/50 p-8 rounded-3xl border border-red-900/30 backdrop-blur-md">
              <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
                <Camera className="h-5 w-5 text-red-500" /> Ingreso de Datos
                Mesa
              </h3>
                  ...
                  <div className="bg-blue-900/10 p-4 rounded-2xl border border-blue-900/20">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData((prev) => ({
                          ...prev,
                          votosCandidato: Number(e.target.value),
                        }))
                      }
                    />
                  </div>
                  <div className="bg-red-900/10 p-4 rounded-2xl border border-red-900/20">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData((prev) => ({
                          ...prev,
                          votosOpositor: Number(e.target.value),
                        }))
                      }
                    />
                  </div>
            ...
            <div className="bg-slate-900/30 rounded-3xl border border-slate-800 p-6 overflow-hidden">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold uppercase tracking-tighter">
                  Últimos E-14 Consolidados
                </h3>
                <button
                  onClick={() => setShowForm(true)}
                  className="text-xs bg-red-600 px-3 py-1 rounded-full font-bold"
                >
                  Nuevo
                </button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {reports
                  ...
                  .map((r) => (
                    <div
                      key={r.id}
                      className="p-4 bg-slate-900 rounded-2xl border border-slate-800 flex justify-between items-center group hover:border-blue-500 transition-all"
                    >
                      <div>
                        <p className="text-[10px] font-bold text-slate-500 uppercase">
                          {r.puesto} • MESA {r.mesa}
                        </p>
            ...
            <div className="h-[200px] w-full">
              ...
              >
                <BarChart data={reports.slice(-5)}>
                  ...
                  />
                  <Bar
                    dataKey="votosCandidato"
                    fill="#3b82f6"
                    radius={[4, 4, 0, 0]}
                  />
                  <Bar
                    dataKey="votosOpositor"
                    fill="#ef4444"
                    radius={[4, 4, 0, 0]}
                  />
                </BarChart>
        ...
        <div className="space-y-6">
          <div className="bg-red-950/20 p-6 rounded-3xl border border-red-900/30">
            ...
            <div className="space-y-4">
              {alerts.map((alert) => (
                <div
                  key={alert.id}
                  className={`p-4 rounded-2xl border ${alert.severity === "high" ? "bg-red-900/20 border-red-900/40 animate-pulse" : "bg-slate-900 border-slate-800"}`}
                >
          ...
          <div className="bg-slate-900/50 p-6 rounded-3xl border border-slate-800 space-y-4">
            <h3 className="text-[10px] font-black text-slate-500 uppercase">
              Estado de Testigos
            </h3>
            <div className="flex items-end gap-1 h-32">
              {BARRIOS.map((_, i) => (
                <div
                  key={i}
                  className="flex-1 bg-blue-600/20 rounded-t hover:bg-blue-600 transition-all"
...
export default function SaaSArchitectureApp() {
  ...
  const triggerRules = (
    ...
    const existingForVoter = currentMissions.filter(
      (m) => m.voterId === voter.id,
    );
    ...
    setState((prev) => {
      ...
      return {
        ...
        logs: [
          {
            id: Date.now(),
            msg: `Votante ${newVoter.name} agregado`,
            time: "0 min",
            type: "info",
          },
          ...prev.logs,
  ...
  const updateVoter = (id: number, voterData: Partial<Voter>) => {
    setState((prev) => {
      const updatedVoters = prev.voters.map((v) =>
        v.id === id ? { ...v, ...voterData } : v,
      );
      const updatedVoter = updatedVoters.find((v) => v.id === id);
      if (updatedVoter) {
        const newMissions = triggerRules(
          updatedVoter,
          prev.missions,
          prev.users,
        );
        return {
          ...
          logs: [
            {
              id: Date.now(),
              msg: `Votante ${updatedVoter.name} actualizado`,
              time: "0 min",
              type: "info",
            },
            ...prev.logs,
  ...
  const massImport = () => {
    const newVoters: Voter[] = Array.from({ length: 1000 }).map((_, i) => {
      ...
      const lider =
        state.users.find((u) => u.role === "LiderZonal" && u.zona === barrio) ||
        state.users[0];
      return {
        id,
        cedula: `80${100000 + i}`,
    ...
    setState((prev) => {
      ...
      return {
        ...
        logs: [
          {
            id: Date.now(),
            msg: `Importación masiva: 1,000 registros cargados`,
            time: "0 min",
            type: "info",
          },
          ...prev.logs,
  ...
  const updateMissionStatus = (
    ...
    setState((prev) => ({
      ...
      logs: [
        {
          id: Date.now(),
          msg: `Misión ${id} actualizada a ${status}`,
          time: "0 min",
          type: "info",
        },
        ...prev.logs,
  ...
  const createCampaign = (campaign: Omit<Campaign, "id" | "status">) => {
    ...
    setState((prev) => ({
      ...
      logs: [
        {
          id: Date.now(),
          msg: `Campaña "${campaign.name}" enviada a ${campaign.audienceCount} personas`,
          time: "0 min",
          type: "info",
        },
        ...prev.logs,
  ...
  const addInteraction = (voterId: number, interaction: Interaction) => {
    setState((prev) => ({
      ...
      voters: prev.voters.map((v) =>
        v.id === voterId
          ? { ...v, interacciones: [interaction, ...v.interacciones] }
          : v,
      ),
    }));
  };
  ...
  const addTransaction = (tx: Omit<Transaction, "id">) => {
    setState((prev) => ({
      ...
      logs: [
        {
          id: Date.now(),
          msg: `Gasto registrado: ${tx.concept}`,
          time: "0 min",
          type: "warning",
        },
        ...prev.logs,
  ...
  const addReport = (
    ...
    setState((prev) => ({
      ...
      logs: [
        {
          id: Date.now(),
          msg: `Reporte recibido Mesa ${report.mesa}`,
          time: "0 min",
          type: "info",
        },
        ...prev.logs,
  ...
  const navigationItems = [
    ...
    if (state.currentUser.role === "Contador")
      return ["dashboard", "finances"].includes(item.id);
    return true;
  });
  ...
  return (
    <AppContext.Provider value={contextValue}>
      <div
        className={`flex h-screen overflow-hidden font-sans ${currentView === "day-d" ? "bg-slate-950" : "bg-slate-50"}`}
      >
        {/* Sidebar */}
        <aside
          className={`${isSidebarOpen ? "w-72" : "w-20"} bg-slate-900 transition-all duration-300 flex flex-col z-30 shadow-2xl`}
        >
          <div className="p-6 flex items-center justify-between border-b border-slate-800">
            {isSidebarOpen && (
              <h1 className="text-xl font-black text-white tracking-tighter italic">
                ESTRATEGIA<span className="text-red-600">2026</span>
              </h1>
            )}
            <button
          ...
          <div className="p-4 border-t border-slate-800">
            {isSidebarOpen && (
              <p className="text-[10px] font-black text-slate-500 uppercase mb-2 px-2">
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          <header
            className={`h-16 flex items-center justify-between px-8 z-20 shadow-sm ${currentView === "day-d" ? "bg-slate-900 border-b border-red-900/30" : "bg-white border-b border-slate-200"}`}
          >
            <div className="flex items-center gap-4">
              <div
                className={`h-2 w-2 rounded-full animate-pulse ${currentView === "day-d" ? "bg-red-500" : "bg-green-500"}`}
              />
              <span
                className={`text-[10px] font-black uppercase tracking-widest ${currentView === "day-d" ? "text-red-500" : "text-slate-500"}`}
              >
                {currentView === "day-d"
            ...
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p
                  className={`text-sm font-black leading-none ${currentView === "day-d" ? "text-white" : "text-slate-900"}`}
                >
                  {state.currentUser.name}
                </p>
                <p
                  className={`text-[10px] font-bold uppercase tracking-tighter ${currentView === "day-d" ? "text-red-500" : "text-blue-600"}`}
                >
                  {state.currentUser.role}
                </p>
              </div>
              <div
                className={`h-10 w-10 rounded-full flex items-center justify-center font-black ${currentView === "day-d" ? "bg-red-600 text-white" : "bg-slate-200 text-slate-600"}`}
              >
          ...
          <div
            className={`flex-1 overflow-y-auto p-4 md:p-8 ${currentView === "day-d" ? "bg-black" : "bg-slate-50"}`}
          >
