    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-slate-500">
          Administra las zonas y asignación de líderes.
        </p>
      </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-2xl font-black tracking-tighter uppercase">
                  Meta Global de Campaña
                </h2>
              </div>
              <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">
                Establece el objetivo de votos fidelizados para la victoria.
              </p>
            </div>
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm sticky top-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      {filteredTeam.length > 0 ? (
                        filteredTeam.map((member) => (
                          ...
                          >
                            ...
                            <span className="text-[10px] font-black text-slate-400 group-hover:text-blue-400 uppercase">
                              {member.role}
                            </span>
                          </button>
                        ))
              ...
              >
                {editingId ? (
                  <>
                    <Edit2 size={16} /> Actualizar Zona
                  </>
                ) : (
                  <>
                    <Plus size={16} /> Guardar Zona
                  </>
                )}
              </button>
            </form>
              ...
              {territory.length === 0 ? (
                <div className="text-center py-12 border-2 border-dashed border-slate-100 rounded-[2rem]">
                  <p className="text-slate-400 font-bold italic text-sm">
                    No hay zonas configuradas. Usa el formulario de la
                    izquierda.
                  </p>
                </div>
              ) : (
                territory.map((zone) => (
                  <div
                    key={zone.id}
                    className="flex items-center justify-between p-5 rounded-[1.5rem] border border-slate-50 hover:border-blue-100 hover:bg-blue-50/10 transition-all group"
                  >
