...
export default function MessagingPage() {
  const { broadcasts, sendBroadcast, updateBroadcast, toggleBroadcastStatus } =
    useCRM();
  ...
  const [broadcastToToggle, setBroadcastToToggle] = useState<Broadcast | null>(
    null,
  );

  const [newCampaign, setNewCampaign] = useState<
    Pick<Broadcast, "name" | "channel" | "segment" | "message">
  >({
    name: "",
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 font-medium">
            Comunicaciones segmentadas y control de envíos.
          </p>
        </div>
        <button
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                ...
                <th className="px-8 py-6 text-[10px] font-black uppercase text-slate-400 tracking-widest text-right">
                  Acciones
                </th>
              </tr>
            </thead>
                ...
                >
                  ...
                  <td className="px-8 py-6">
                    ...
                    <p className="text-[10px] font-bold text-emerald-600">
                      {b.deliveredCount.toLocaleString()} Entregados
                    </p>
                  </td>
            ...
            <form onSubmit={handleSubmit} className="p-10 space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div className="col-span-2">
                  ...
                  <input
                    required
                    placeholder="Ej: Invitación Votación Domingo"
                    className="w-full px-5 py-3 border-2 border-slate-100 rounded-2xl text-sm font-bold focus:border-blue-500 outline-none transition-all"
                ...
                <div>
                  ...
                  <select
                    className="w-full px-5 py-3 border-2 border-slate-100 rounded-2xl text-sm font-bold focus:border-blue-500 outline-none"
                    value={newCampaign.channel}
                    onChange={(e) =>
                      setNewCampaign({
                        ...newCampaign,
                        channel: e.target.value as Broadcast["channel"],
                      })
                    }
                  >
                ...
                <div className="col-span-2">
                  ...
                  <textarea
                    ...
                    className="w-full px-5 py-3 border-2 border-slate-100 rounded-2xl text-sm font-medium focus:border-blue-500 outline-none"
              ...
              <div className="pt-4 flex gap-4">
                <button
                  type="button"
                  onClick={closeModal}
                  className="flex-1 px-4 py-4 border-2 border-slate-100 rounded-2xl text-xs font-black uppercase text-slate-600 hover:bg-slate-50 transition-all"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  className="flex-1 px-4 py-4 bg-blue-600 rounded-2xl text-xs font-black uppercase text-white shadow-xl hover:bg-blue-700 transition-all flex items-center justify-center gap-2"
                >
                  <Send size={16} />{" "}
                  {editingBroadcast ? "Guardar Cambios" : "Lanzar Campaña"}
                </button>
              </div>
            </form>
          </div>
      ...
      <AlertDialog
        ...
        variant={
          broadcastToToggle?.activeStatus === "active" ? "warning" : "info"
        }
      />
    </div>
  );
