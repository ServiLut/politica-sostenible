...
export default function PipelinePage() {
  ...
  const filteredContacts = useMemo(() => {
    return contacts.filter((c) => {
      ...
      const matchesLeader =
        selectedLeader === "all" || c.neighborhood.includes(selectedLeader);
      return matchesSearch && matchesLeader;
    });
  }, [contacts, searchTerm, selectedLeader]);

  // Cálculos de Estrategia
  const confirmedVotes = contacts.filter(
    (c) => c.stage === "Firme" || c.stage === "Votó",
  ).length;
  const targetVotes = campaignGoal;
  const progressPercent =
    targetVotes > 0 ? (confirmedVotes / targetVotes) * 100 : 0;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium">
            Mueve a los ciudadanos hacia el voto seguro.
          </p>
        </div>
        <div className="flex gap-3 w-full md:w-auto">
          ...
          >
            ...
            {team.map((m) => (
              <option key={m.id} value={m.territory}>
                {m.name} ({m.territory})
              </option>
            ))}
          </select>
      ...
      <div className="flex-1 overflow-x-auto pb-6 -mx-6 px-6 scrollbar-hide">
        <div className="flex gap-6 h-full min-w-max">
          {STAGES.map((stage, index) => {
            ...
            const nextStageCount = filteredContacts.filter(
              (c) => c.stage === nextStage,
            ).length;
            ...
            const convRate =
              stageContacts.length > 0
                ? Math.round(
                    (nextStageCount / (stageContacts.length + nextStageCount)) *
                      100,
                  )
                : 0;
                      ...
                      >
                        <div className="flex justify-between items-start mb-4">
                          ...
                          <div className="flex gap-1">
                            <button
                              onClick={() =>
                                handleMove(contact.id, stage, "prev")
                              }
                            ...
                            <button
                              onClick={() =>
                                handleMove(contact.id, stage, "next")
                              }
                              disabled={
                        ...
                        <h4 className="text-sm font-black text-slate-900 mb-4 tracking-tight leading-tight uppercase">
                          {contact.name}
                        </h4>
