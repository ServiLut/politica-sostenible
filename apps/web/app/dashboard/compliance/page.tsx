...
export default function CompliancePage() {
  const { compliance, getComplianceScore, uploadEvidence, logAction } =
    useCRM();
  ...
  const handleUpload = () => {
    if (selectedId) {
      ...
      logAction(
        "SuperAdmin",
        `Cargó soporte para obligación ID: ${selectedId}`,
        "Compliance",
        "Info",
      );
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-slate-500">
            Blindaje normativo y control ante el Consejo Nacional Electoral.
          </p>
        </div>
        <button
          onClick={() => {
            logAction(
              "SuperAdmin",
              "Generó reporte consolidado CNE",
              "Compliance",
              "Warning",
            );
            alert(
              "Generando Reporte Oficial CNE... Formularios 5A, 5B y 5C en proceso.",
            );
          }}
        ...
        <div className="lg:col-span-2 bg-white border border-slate-100 rounded-[3rem] overflow-hidden shadow-sm">
          <div className="px-10 py-8 border-b border-slate-50 bg-slate-50/50 flex justify-between items-center">
            <h3 className="font-black text-slate-900 flex items-center gap-2">
              <FileText size={20} className="text-blue-600" /> Matriz de
              Obligaciones
            </h3>
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
              {compliance.length} Pendientes
            </span>
          </div>
          ...
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-8 space-y-6 text-center">
              ...
              <h3 className="text-2xl font-black text-slate-900 tracking-tighter">
                Cargar Evidencia Legal
              </h3>
              <p className="text-slate-500 text-sm">
                Selecciona el archivo PDF o imagen que soporta el cumplimiento
                de esta obligación ante el CNE.
              </p>
              ...
              <div className="flex gap-4">
                ...
                <button
                  ...
                  className="flex-1 py-4 bg-blue-600 text-white rounded-2xl text-xs font-black uppercase shadow-lg"
                >
                  Confirmar Carga
                </button>
              </div>
