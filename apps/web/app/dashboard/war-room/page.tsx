    ...
    <div className="space-y-8 pb-20 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        ...
        <div className="grid grid-cols-2 gap-3 md:gap-4 w-full lg:w-auto">
           <div className="bg-white p-4 px-5 md:px-6 rounded-2xl md:rounded-[1.5rem] border-2 border-slate-100 shadow-sm flex flex-col justify-center">
              <p className="text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Mesas Procesadas</p>
              <div className="flex items-end gap-1.5 md:gap-2">
                 <span className="text-xl md:text-2xl font-black text-slate-900 leading-none">{tablesReported}</span>
                 <span className="text-[10px] md:text-xs font-bold text-slate-300 mb-0.5">/ {totalTables}</span>
              </div>
           </div>
           <div className="bg-teal-600 p-4 px-5 md:px-6 rounded-2xl md:rounded-[1.5rem] shadow-xl shadow-teal-100 flex flex-col justify-center">
              <p className="text-[8px] md:text-[9px] font-black text-teal-100/60 uppercase tracking-widest mb-1 text-center">Avance Nacional</p>
              <p className="text-xl md:text-2xl font-black text-white leading-none text-center">{reportedPercentage}%</p>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
         ...
         <div className="bg-white p-6 rounded-[1.5rem] md:rounded-[2rem] border-2 border-slate-100 shadow-sm flex items-center gap-4 md:gap-5 group hover:border-indigo-500/30 transition-all">
            ...
            <div>
               ...
               <h4 className="text-lg md:text-xl font-black text-slate-900 leading-tight">{(myVotes + opponentVotes).toLocaleString()}</h4>
            </div>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-9 bg-white rounded-[2rem] md:rounded-[3.5rem] p-8 md:p-16 border-2 border-slate-100 shadow-2xl relative overflow-hidden flex flex-col justify-center">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 relative z-10">
            ...
            <div className="space-y-4 md:space-y-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                   <p className="text-[10px] md:text-xs font-black uppercase tracking-[0.3em] text-teal-600">Candidato SIT</p>
            ...
            {/* Oponente */}
            <div className="space-y-4 md:space-y-6 md:text-right flex flex-col md:items-end">
              <div className="flex items-center gap-3 md:flex-row-reverse">
                ...
                <div>
                   <p className="text-[10px] md:text-xs font-black uppercase tracking-[0.3em] text-rose-500">Opositor Principal</p>
          ...
          <div className="mt-12 md:mt-20 space-y-4 md:space-y-6 relative z-10">
             <div className="flex justify-between items-center px-2">
                <div className="flex items-center gap-2">
                   <div className="w-2 h-2 rounded-full bg-teal-500 animate-ping" />
                   <p className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.4em] text-teal-600">Ventaja</p>
                </div>
                <div className="px-4 md:px-5 py-1.5 md:py-2 bg-slate-900 text-white rounded-full text-[10px] md:text-xs font-black uppercase tracking-widest shadow-2xl">
                  {lead >= 0 ? '+' : ''}{lead.toLocaleString()}
                </div>
                <p className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.4em] text-rose-400 text-right">Contención</p>
             </div>
             <div className="h-8 md:h-10 w-full flex rounded-full overflow-hidden shadow-2xl p-1 bg-white border-2 border-slate-100">
                 ...
                 <div className="relative w-36 h-36 md:w-40 md:h-40 mx-auto mb-8">
                    <svg className="w-full h-full transform -rotate-90">
                      ...
                      <circle cx="72" cy="72" r="66" stroke="currentColor" strokeWidth="10" fill="transparent" 
                        strokeDasharray={414.7} strokeDashoffset={414.7 - (414.7 * reportedPercentage) / 100}
                        strokeLinecap="round"
                        className="text-teal-500 transition-all duration-1000 md:hidden" 
                      />
                      <circle cx="80" cy="80" r="72" stroke="currentColor" strokeWidth="10" fill="transparent" 
                        ...
                        strokeLinecap="round"
                        className="text-teal-500 transition-all duration-1000 hidden md:block" 
                      />
                    </svg>
      ...
      <div className="pt-8 space-y-8">
        <div className="bg-white p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border-2 border-slate-100 shadow-sm flex flex-col xl:flex-row justify-between items-center gap-6 md:gap-8">
          <div className="flex items-center gap-4 w-full xl:w-auto">
            ...
            <div>
              ...
              <p className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Gestión de Actas E-14 por Puesto</p>
            </div>
          </div>

          <div className="flex flex-col md:flex-row items-center justify-center gap-4 w-full xl:w-auto">
            <div className="flex bg-slate-100 p-1.5 rounded-2xl md:rounded-[1.25rem] border border-slate-200 shadow-inner overflow-x-auto w-full md:w-auto no-scrollbar">
               {[
                 { id: 'all', label: 'Todos' },
        ...
        <div className="grid grid-cols-1 gap-6 md:gap-8">
          {loadingStations ? (
            ...
            <div className="py-20 flex flex-col items-center justify-center text-rose-500 bg-white rounded-[2.5rem] md:rounded-[3.5rem] border-2 border-slate-100 shadow-sm">
            ...
            <div className="bg-white rounded-[2.5rem] md:rounded-[3.5rem] p-16 md:p-24 text-center border-2 border-dashed border-slate-200">
              <div className="w-20 h-20 md:w-24 md:h-24 bg-slate-50 rounded-[2rem] md:rounded-[2.5rem] flex items-center justify-center mx-auto mb-8 border border-slate-100">
                <Search size={32} className="md:size-10 text-slate-200" />
              </div>
              <h4 className="text-lg md:text-xl font-black text-slate-900 uppercase tracking-tighter">Sin resultados operacionales</h4>
            </div>
          ) : (
                  ...
                  <div className="flex flex-col lg:grid lg:grid-cols-12 items-stretch">
                    <div className="lg:col-span-3 p-8 md:p-10 flex flex-col justify-center bg-slate-50/30 border-b lg:border-b-0 lg:border-r border-slate-100">
                       <div className="flex items-center gap-4 md:gap-5 mb-6">
                          <div className={cn(
                            "w-16 h-16 md:w-20 md:h-20 rounded-2xl md:rounded-[2.25rem] flex flex-col items-center justify-center shrink-0 border-2 md:border-4 transition-all shadow-xl",
                            ...
                            <span className="text-xl md:text-2xl font-black leading-none">{percentage}%</span>
                            <span className="text-[7px] md:text-[8px] font-black uppercase tracking-tighter opacity-80">Sync</span>
                          </div>
                          <div className="min-w-0">
                             <h4 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight uppercase group-hover:text-teal-600 transition-colors leading-tight truncate">
                               {station.nombre || station.puesto}
                             </h4>
                             <p className="text-[8px] md:text-[9px] font-bold text-slate-400 uppercase tracking-widest truncate">{station.municipio}, {station.departamento}</p>
                          </div>
                       </div>
                    ...
                    <div className="lg:col-span-4 p-8 md:p-10 flex flex-col justify-center border-b lg:border-b-0 lg:border-r border-slate-100 space-y-5 md:space-y-6">
                       <div className="flex justify-between items-end mb-1 md:mb-2 px-2">
                          <p className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Escenario Local</p>
                          <span className={cn(
                            "px-2 md:px-3 py-1 rounded-lg text-[9px] md:text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 md:gap-2",
                       ...
                       <div className="grid grid-cols-2 gap-3 md:gap-4">
                          <div className="bg-teal-50/50 p-3 md:p-4 rounded-xl md:rounded-2xl border border-teal-100/50">
                             <p className="text-[8px] md:text-[9px] font-black text-teal-600 uppercase mb-1">SIT</p>
                             <p className="text-xl md:text-2xl font-black text-teal-900 leading-none">{stationMyVotes.toLocaleString()}</p>
                          </div>
                          <div className="bg-rose-50/30 p-3 md:p-4 rounded-xl md:rounded-2xl border border-rose-100/50">
                             <p className="text-[8px] md:text-[9px] font-black text-rose-500 uppercase mb-1">Opo</p>
                             <p className="text-xl md:text-2xl font-black text-rose-900 leading-none">{stationOpponentVotes.toLocaleString()}</p>
                          </div>
                       </div>
                    ...
                    <div className="lg:col-span-5 p-8 md:p-10 flex flex-col">
                       ...
                       <div className="mt-4 md:mt-6 pt-4 md:pt-6 border-t border-slate-50 flex justify-end">
                          <button className="flex items-center gap-2 text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 hover:text-teal-600 transition-colors">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between bg-white p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border-2 border-slate-100 shadow-sm mt-8 gap-4">
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              <button
                ...
                className="flex-1 sm:flex-none p-3 md:p-4 bg-slate-50 border-2 border-slate-100 rounded-xl md:rounded-2xl text-slate-400 hover:text-teal-600 disabled:opacity-30 transition-all hover:bg-white hover:border-teal-500 shadow-sm"
              >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[200] flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] md:rounded-[4rem] shadow-2xl overflow-hidden border-4 md:border-[8px] border-white ring-1 ring-slate-200 max-h-[95vh] overflow-y-auto">
            <div className="p-8 md:p-12 pb-6 md:pb-8 flex justify-between items-start">
              <div className="space-y-1 md:space-y-2">
                <div className="flex items-center gap-2 md:gap-3 mb-1 md:mb-2">
                   <div className="px-2 md:px-3 py-0.5 md:py-1 bg-slate-900 text-white text-[8px] md:text-[9px] font-black uppercase rounded-lg">E-14</div>
                   <span className="text-[10px] md:text-xs font-black text-teal-600 uppercase">Auditoría SIT</span>
                </div>
                <h3 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tighter leading-none uppercase italic">Votos <span className="text-teal-600">Mesa</span></h3>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="p-3 md:p-5 bg-slate-100 text-slate-400 hover:text-rose-600 rounded-full transition-all group shrink-0"><X size={20} className="md:size-7" /></button>
            </div>
            <form onSubmit={handleReport} className="p-8 md:p-12 pt-0 space-y-8 md:space-y-12">
              <div className="flex items-center gap-4 md:gap-6 bg-slate-50 p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border-2 border-slate-100">
                <div className="w-14 h-14 md:w-20 md:h-20 bg-white rounded-xl md:rounded-[1.75rem] flex items-center justify-center text-slate-900 border-2 border-slate-200 shadow-xl font-black text-xl md:text-3xl italic shrink-0">{newReport.tableNumber}</div>
                <div className="min-w-0">
                  <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase mb-1 truncate">Estación</p>
                  <h4 className="text-lg md:text-2xl font-black text-slate-900 uppercase truncate">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                <div className="space-y-3 md:space-y-5">
                  <label className="text-[10px] md:text-[11px] font-black uppercase text-teal-600 px-4">Candidato SIT</label>
                  <input required type="number" min="0" autoFocus className="w-full px-6 md:px-8 py-8 md:py-12 bg-teal-50/50 border-4 md:border-[6px] border-teal-100 rounded-[2.5rem] md:rounded-[3.5rem] text-5xl md:text-7xl font-black text-teal-900 focus:border-teal-500 focus:bg-white outline-none transition-all text-center" value={newReport.votesCandidate || ''} onChange={e => setNewReport({...newReport, votesCandidate: Number(e.target.value)})} />
                </div>
                <div className="space-y-3 md:space-y-5">
                  <label className="text-[10px] md:text-[11px] font-black uppercase text-rose-500 px-4">Oposición</label>
                  <input required type="number" min="0" className="w-full px-6 md:px-8 py-8 md:py-12 bg-rose-50/30 border-4 md:border-[6px] border-rose-100 rounded-[2.5rem] md:rounded-[3.5rem] text-5xl md:text-7xl font-black text-rose-900 focus:border-rose-500 focus:bg-white outline-none transition-all text-center" value={newReport.votesOpponent || ''} onChange={e => setNewReport({...newReport, votesOpponent: Number(e.target.value)})} />
                </div>
              </div>
              <div className="flex flex-col sm:flex-row gap-4 md:gap-6 pt-2 md:pt-4">
                <button type="button" onClick={() => setIsModalOpen(false)} className="order-2 sm:order-1 flex-1 px-8 py-6 md:py-8 bg-slate-50 text-slate-400 rounded-[1.5rem] md:rounded-[2rem] text-[10px] md:text-xs font-black uppercase hover:bg-slate-100 transition-all">Descartar</button>
                <button type="submit" disabled={reportMutation.isPending} className="order-1 sm:order-2 flex-[2] px-8 py-6 md:py-8 bg-slate-900 text-white rounded-[1.5rem] md:rounded-[2rem] text-xs md:text-sm font-black uppercase shadow-2xl hover:bg-teal-600 transition-all flex items-center justify-center gap-3 md:gap-4">
                  {reportMutation.isPending ? <div className="w-5 h-5 md:w-6 md:h-6 border-4 border-white border-t-transparent rounded-full animate-spin" /> : <ShieldCheck size={20} className="md:size-6 text-teal-400" />}
                  Confirmar
                </button>
              </div>
            </form>
