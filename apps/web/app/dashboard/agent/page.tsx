  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Hola, soy tu estratega digital. ¿En qué puedo ayudarte hoy con la campaña? He analizado los datos más recientes y tengo algunas sugerencias interesantes para la zona norte.",
      timestamp: new Date(),
    },
  ]);
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      const aiMsg: Message = {
        role: "assistant",
        content:
          "Entendido. Estoy procesando esa información con los datos históricos de 2022 y las tendencias actuales. Dame un momento para generarte un reporte detallado.",
        timestamp: new Date(),
      };
            ...
            >
              ...
              {msg.role === "assistant" && (
                <div className="flex items-center gap-2 px-2">
                  <button className="p-1.5 text-zinc-300 hover:text-emerald-500 transition-colors">
                    <ThumbsUp className="h-3 w-3" />
                  </button>
                  <button className="p-1.5 text-zinc-300 hover:text-red-500 transition-colors">
                    <ThumbsDown className="h-3 w-3" />
                  </button>
                  <button className="p-1.5 text-zinc-300 hover:text-primary transition-colors">
