...
export function TacticalMap({ zones }: TacticalMapProps) {
  return (
    <div className="relative w-full aspect-[16/9] bg-[#0F172A] rounded-[3rem] overflow-hidden border border-white/10 shadow-2xl group">
      ...
      >
        <path
          d="M100,100 Q150,50 200,100 T300,100 T400,150 T500,100"
          fill="none"
          stroke="white"
          strokeWidth="2"
        />
        <path
          d="M50,300 Q150,250 250,300 T450,350 T650,300"
          fill="none"
          stroke="white"
          strokeWidth="1"
        />
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="relative w-full h-full">
          ...
          {zones.map((zone, idx) => {
            ...
            const left = zone.coords
              ? `${(zone.coords[1] + 76) * 500}%`
              : `${20 + idx * 25}%`;
            const top = zone.coords
              ? `${(7 - zone.coords[0]) * 500}%`
              : `${30 + (idx % 2) * 30}%`;
            ...
            return (
              ...
              >
                ...
                <div
                  className={cn(
                    "absolute inset-0 w-12 h-12 -m-6 rounded-full animate-ping opacity-20",
                    colorClass,
                  )}
                />
                ...
                <div
                  className={cn(
                    "relative h-4 w-4 rounded-full border-4 border-[#0F172A] shadow-lg",
                    colorClass,
                  )}
                />
